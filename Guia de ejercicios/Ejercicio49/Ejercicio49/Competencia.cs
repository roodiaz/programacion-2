...
namespace Ejercicio46
{
        ...
        where T: VehiculoDeCarrera
    {
        ...
        public static bool operator ==(Competencia <T> c, VehiculoDeCarrera v)
        {
            ...
            {
                if (c.Tipo == Competencia<T>.TipoCompetencia.F1 && v.GetType() != typeof(AutoF1) ||
                    c.Tipo == Competencia<T>.TipoCompetencia.MotoCross && v.GetType() != typeof(MotoCross))
                {
        ...
        public static bool operator -(Competencia <T> c, VehiculoDeCarrera v)
        {
            if (!(c.competidores is null) && !(v is null))
            {
                if (c == v)
                {
                    c.competidores.RemoveAt(c.competidores.IndexOf((T)v));
                    return true;
                }
            }
